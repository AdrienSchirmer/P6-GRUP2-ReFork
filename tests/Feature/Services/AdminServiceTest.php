...
test('admin can create a valid service', function () {
    ...
    $response = $this->post('/admin/services', [
        ...
        'description' => 'Atenció general per a tots els pacients.',
        'duration_minutes' => 30,
        'icon' => 'stethoscope',
    ]);
...
test('service creation fails with invalid name (digits not allowed)', function () {
    $this->actingAs(adminUser());

    $response = $this->from('/admin/services/create')->post('/admin/services', [
        'name' => 'Consulta 123', // digits not allowed by the regex
    ...
    $this->assertDatabaseMissing('services', ['name' => 'Consulta 123']);
});
