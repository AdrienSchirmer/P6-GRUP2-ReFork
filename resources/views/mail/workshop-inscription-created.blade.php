...
<head>
    <meta charset="UTF-8">
    <title>Confirmació d'inscripció al taller</title>
</head>
...
<div style="width:100%;padding:30px 0;">
    <div style="width:600px;margin:0 auto;background:#fff;border-radius:10px;overflow:hidden;">
        <div style="background:#0f5f7f;color:#fff;padding:24px;text-align:center;">
            <span style="display:inline-block;background:#dcfce7;color:#166534;padding:6px 14px;border-radius:20px;font-size:11px;font-weight:bold;">Inscripció confirmada</span>
            <h1 style="margin:8px 0 4px 0;font-size:20px;">Gràcies per inscriure't!</h1>
            <p style="margin:0;font-size:12px;opacity:0.9;">{{ $data['pharmacy'] }}</p>
        </div>
        <div style="padding:24px;color:#0f172a;">
            <p style="font-size:14px;margin:0 0 10px 0;">Hola <strong>{{ $data['name'] }}</strong>,</p>
            <p style="font-size:13px;color:#555;margin:0 0 18px 0;">
                Hem rebut correctament la teva inscripció al taller
                <strong>{{ $data['workshop_name'] }}</strong>.
            </p>
            ...
            <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:14px;margin-bottom:16px;font-size:13px;">
                <div style="padding:4px 0;"><span style="color:#64748b;">Nom:</span> {{ $data['name'] }}</div>
            ...
            <p style="font-size:12px;color:#555;">
                T'esperem a <strong>{{ $data['address'] }}</strong>.<br>
                Si tens cap dubte, truca'ns al {{ $data['phone_pharmacy'] }}.
            </p>
        </div>
        <div style="background:#f1f5f9;color:#64748b;padding:16px 24px;font-size:11px;text-align:center;">
            {{ $data['pharmacy'] }} — Missatge automàtic, no responguis aquest correu.
        </div>
    </div>
</div>
</body>
